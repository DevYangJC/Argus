...
elasticsearch:
  ...
  port: 9200
  scheme: http
  index-name: argus_rag_document_chunks
```
...
psql -h 192.168.200.130 -U root -d argus_rag -c "CREATE EXTENSION IF NOT EXISTS vector;"
psql -h 192.168.200.130 -U root -d argus_rag -f sql/schema.sql
```
...
| 文件 | 端口 | 数据库 | 中间件 | 说明 |
|------|------|------|------|------|
| `application-local.yml` | 10001 | 192.168.200.130:5433/argus_rag | MinIO: :9000, ES: :9200 | 本地开发（默认） |
