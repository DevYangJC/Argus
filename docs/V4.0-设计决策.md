...
sequenceDiagram
    ...
    rect rgb(240, 255, 240)
        ...
        rect rgb(240, 255, 255)
            ...
            Tool->>Retriever: retrieveEvidence(groupId=1, query)
            Retriever-->>Tool: RetrievedEvidenceBundle (documents + evidenceLevel)
...
工具调用走 V3.0 的完整检索链路（`ReadyChunkDocumentRetriever` → `HybridChunkRetrievalService` → 查询规划 + 双通道检索 + RRF 融合 + 证据评估）。
...
1. **检索已经做了**——`ReadyChunkDocumentRetriever.retrieveEvidence()` 执行了完整的查询规划 + 双通道检索 + RRF 融合 + 证据评估，这已经是一次高质量的检索。
