...
**access token** 是一个 JWT，采用 HMAC-SHA256 签名，有效期为 30 分钟（可通过 `argus.rag.auth.jwt.expiration` 配置）。由于 JWT 本身包含了用户 ID、角色等基本信息，服务端无需查询数据库即可完成认证，这使得每次请求的认证开销极低。access token 通过 HTTP 的 `Authorization: Bearer <token>` 头传输，前端存储于内存中。
...
| 文件 | 端口 | 数据库 | 说明 |
|------|------|------|------|
| `application-local.yml` | 10001 | 192.168.200.130:5433/argus_rag | 本地开发（默认） |
