...
import com.argus.rag.document.service.DocumentService;
...
public class DocumentController {
    ...
    /** 文档核心服务，处理直接上传、查询、删除、预览、下载等操作 */
    private final DocumentService documentService;
    /** 分片上传服务，处理大文件的分片初始化、分片上传、合并完成等操作 */
    private final DocumentUploadService documentUploadService;

    /**
     * 构造文档控制器，注入文档服务和分片上传服务。
     *
     * @param documentService       文档核心服务
     * @param documentUploadService 分片上传服务
     */
     ...
     * 文件大小限制为 10MB，支持 txt / md / pdf / docx 格式。
     * 需要调用者是群组管理员。
     *
    ...
    public ApiResponse<List<DocumentListItemVO>> listDocuments(
        ...
        return ApiResponse.success(documentService.listDocuments(request, query));
    }
    ...
    public ApiResponse<DocumentPreviewVO> previewDocument(
        ...
        return ApiResponse.success(documentService.previewDocument(request, groupId, documentId));
    }
    ...
    public ResponseEntity<InputStreamResource> downloadDocument(
        ...
        DocumentDownloadVO downloadInfo = documentService.downloadDocument(request, groupId, documentId);
